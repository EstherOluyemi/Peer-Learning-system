...
// Get session chat messages (unified endpoint for both tutors and learners)
export const getSessionChat = async (sessionId, options = {}) => {
  try {
    const { page = 1, limit = 50 } = options;
    // Use unified chat endpoint that works for both tutors and learners
    const response = await api.get(`/v1/chat/sessions/${sessionId}/chat`, {
      params: { page, limit }
    });
    const payload = response?.data ?? response;
...
// Send a message to session chat (unified endpoint for both tutors and learners)
export const sendSessionChatMessage = async (sessionId, messageText) => {
  try {
    // Use unified chat endpoint that works for both tutors and learners
    const response = await api.post(`/v1/chat/sessions/${sessionId}/chat`, {
      message: messageText
    });
    const payload = response?.data ?? response;
